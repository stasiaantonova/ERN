...
receivers:
  - name: 'telepush'
    webhook_configs:
      - url: 'https://telepush.dev/api/inlets/alertmanager/$TOKEN_telegrambot'
inhibit_rules:
  - source_match:
      severity: 'critical'
    target_match:
...
sudo tee vim /etc/prometheus/rules.yml > /dev/null <<EOF
groups:
  - name: alert_rules
    rules:
      - alert: $Node_name
...
scrape_configs:
  - job_name: "$Node_name"
    scrape_interval: 5s
    static_configs:
      - targets: ["$IP_server:9090","$IP_server:9615"]
EOF
